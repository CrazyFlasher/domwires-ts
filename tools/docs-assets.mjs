...
for (const [name, c] of Object.entries(themes))
{
    ...
    let flow = rect(0.5, 0.5, 1199, 819, c.bg, c.line, 20) + arrows(c) + title;
    flow += rect(24, 113, 1152, 634, c.panel, c.line, 18) + text(45, 143, "GameContext", 17, c.muted, 600);
    flow += rect(297, 322, 859, 397, c.soft, c.line, 16) + text(317, 352, "SceneContext", 17, c.teal, 650);
    flow += box(c, 48, 177, 186, "GameView", "DOM click");
    flow += box(c, 322, 177, 262, "GameMediator", "dispatchMessage(FIRE)");
    flow += box(c, 787, 177, 339, "Message routing", "Explicit child receive predicate");
    flow += path("M234 218 H310", c) + text(240, 201, "callback", 14, c.teal, 650);
    flow += path("M584 218 H775", c) + text(621, 201, "FIRE · bubbles", 14, c.teal, 650);
    flow += text(48, 286, "GameMediator creates and owns GameView", 16, c.muted);
    flow += path("M950 259 V297 H511 V372", c) + text(609, 286, "receive FIRE", 14, c.teal);
    flow += box(c, 322, 384, 210, "Message mapping", "map(FIRE, Fire)");
    flow += box(c, 622, 384, 184, "Fire", "Command", c.violet);
    flow += box(c, 896, 384, 228, "SceneModel", "Owns scene state");
    flow += path("M532 425 H610", c, 1) + text(551, 408, "run", 14, c.violet);
    flow += path("M806 425 H884", c, 1) + text(823, 408, "fire()", 14, c.violet);
    flow += box(c, 322, 593, 210, "SceneView", "Canvas rendering");
    flow += box(c, 622, 593, 234, "SceneMediator", "Reads Immutable state");
    flow += path("M1006 466 V556 H739 V581", c);
    flow += text(687, 531, "STATE_CHANGED · context forwards", 15, c.teal, 600);
    flow += path("M622 634 H544", c, 1) + text(536, 617, "render(data)", 14, c.violet);
    flow += path("M856 634 H1104 V478", c, 2, true);
    flow += text(876, 617, "read access", 16, c.muted);
    flow += text(322, 700, "SceneMediator creates and owns SceneView. Views render data and report input.", 16, c.muted);
    flow += text(40, 793, "Solid: callbacks / messages / execution  ·  Dashed: read access  ·  Based on examples/game", 16, c.muted);
    writeFileSync(output + `message-flow-${name}.svg`, svg(1200, 820, "DomWires Fire message flow", "GameContext owns GameMediator, which creates and owns GameView. GameView reports a click to the mediator. The mediator dispatches FIRE into SceneContext. Fire changes SceneModel. STATE_CHANGED reaches SceneMediator, which reads the live Immutable model and updates its owned SceneView.", flow));
    ...
    contexts += text(45, 177, "Owns the clock and GameMediator; GameMediator creates and owns GameView", 17, c.muted);
    ...
    contexts += box(c, 405, 545, 276, "SceneMediator", "Creates and owns SceneView");
    ...
    contexts += box(c, 840, 545, 286, "HudMediator", "Creates and owns HudView");
